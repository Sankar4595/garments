...
const Grouped: React.FC<Props> = ({ data, productId }) => {
  ...
  const handleAddToCart = () => {
    if (
      !cartState.cartArray.find(
        (item) =>
          item._id === productMain._id ||
          item._id === data[Number(productId)]._id ||
          item._id === data[Number(productId) + 1]._id ||
          item._id === data[Number(productId) + 2]._id
      )
    ) {
      ...
      updateCart(productMain._id, quantity.id, activeSize, activeColor);
      updateCart(
        data[Number(productId)]._id,
        quantity.id,
        activeSize,
        activeColor
      );
      updateCart(
        data[Number(productId) + 1]._id,
        quantity.id,
        activeSize,
        activeColor
      );
      updateCart(
        data[Number(productId) + 2]._id,
        quantity.id,
        activeSize,
        activeColor
      );
  ...
  const handleAddToCompare = () => {
    ...
    if (compareState.compareArray.length < 3) {
      if (
        compareState.compareArray.some((item) => item._id === productMain._id)
      ) {
